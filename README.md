...
Alright, let's go step by step. We'll make sure everything is done properly before moving forward.  

### **Step 1: Update `main.tf`**  
This file will store all our variable declarations to make our configuration cleaner.  

#### **Actions:**  
1. Navigate to your Terraform project directory (e.g., `PBL`).  
2. Create a new file named **`main.tf`**.  
3. Copy the following variable declarations into `main.tf`:  
...
![image](https://github.com/user-attachments/assets/b444c996-8885-411e-9709-3c39fcf3b6f7)



### **Step 2: Create `variables.tf`**  

Then, open `variables.tf` in your editor and paste the following content:
...
![image](https://github.com/user-attachments/assets/aee2bd04-9ec7-4e18-b8fa-76bfb9db72a6)


### **Step 3: Create `terraform.tfvars` 
Since we have moved our variable declarations to `variables.tf`, we need to update `main.tf` to reference those variables properly.

#### **Actions:**  
1. Open `terraform.tfvars` in your editor and paste the following values:
...
enable_dns_support = true
enable_dns_hostnames = true
public_subnet1_cidr = "172.16.0.0/24"
public_subnet2_cidr = "172.16.1.0/24"
availability_zone_1 = "us-east-1a"
availability_zone_2 = "us-east-1b"

```

![image](https://github.com/user-attachments/assets/b5168728-c4c0-4866-87e4-3d2d39ddcc5e)


### Now, let's verify everything works with `terraform plan`.

### **Step 5: Run `terraform plan`**  
Before running the plan, make sure you're in the project directory (`PBL`) where your `main.tf`, `variables.tf`, and `terraform.tfvars` files are located.
...
This will initialize the Terraform project and download the necessary provider plugins.

Then, run the `terraform plan` command:  

```sh
terraform plan
```

This will show you a preview of the actions Terraform will take based on your configuration. It should not make any changes yet but will display the resources that will be created, modified, or destroyed.

![image](https://github.com/user-attachments/assets/2fedfce5-d3fc-422e-b777-a6f1bdc2f050)

***If `terraform plan` looks good and shows no errors, you’re all set!***


# ***Conclusion***  

This project demonstrated the importance of structuring Terraform configurations for better readability, maintainability, and scalability. By separating variable declarations into `variables.tf` and storing variable values in `terraform.tfvars`, we achieved a more modular and flexible approach to infrastructure as code (IaC).  

Key takeaways from this project include:  
1. **Improved Code Organization** – Moving variable definitions to separate files reduces clutter in `main.tf`, making it easier to read and manage.  
2. **Enhanced Reusability** – By defining variables externally, the same configuration can be reused across different environments with minimal modifications.  
3. **Simplified Configuration Management** – Using `terraform.tfvars` allows for quick updates without altering the core Terraform code.  
4. **Scalability and Best Practices** – This structured approach aligns with Terraform best practices, making infrastructure deployment more efficient and scalable.  

***By following this approach, we have created a well-structured Terraform setup that simplifies AWS infrastructure provisioning while ensuring flexibility for future changes.***
